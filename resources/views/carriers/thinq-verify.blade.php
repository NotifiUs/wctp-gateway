    ...
    <div class="row justify-content-start mb-2">
        @if( $account['balance'] === 0 )
            <div class="alert alert-warning w-100" role="alert">
                <i class="fas fa-exclamation-triangle"></i> This account has a <strong>zero balance</strong>. Add funds before going into production!
            </div>
        @endif
                            ...
                            <div class="col-12 col-xl-6">
                                ...
                                <dl class="w-100 bg-light rounded p-4">
                                    ...
                                    <dl class="text-dark font-weight-bolder">
                                        {{ $account['account_id'] }}
                                        <input type="hidden" name="thinq_account_id" value="{{ $account['account_id'] }}">
                                    </dl>
                                    <dt class="text-muted-light">API Username</dt>
                                    <dl class="">
                                        <code class="text-primary font-weight-bold">{{ $account['api_username'] }}</code>
                                        <input type="hidden" name="thinq_api_username" value="{{ $account['api_username'] }}">
                                    </dl>
                                    <dt class="text-muted-light">API Token</dt>
                                    <dl class="">
                                        <code class="text-primary font-weight-bold">{{ str_pad( substr( $account['api_token'],0, 6), 40, '*') }}</code>
                                        <input type="hidden" name="thinq_api_token" value="{{  $account['api_token'] }}">
                                    </dl>
