                            ...
                            <div class="dropdown float-right">
                                ...
                                <div class="dropdown-menu dropdown-menu-right shadow-sm bg-light"
                                    ...
                                    <a class="dropdown-item" href="#" data-toggle="modal"
                                       data-target="#editCarrierModal{{ $carrier->id }}">{{ __('Edit Settings') }}</a>
                                    <a class="dropdown-item" href="#" data-toggle="modal"
                                       data-target="#credentialsCarrierModal{{ $carrier->id }}">{{ __('View Credentials') }}</a>
                                    <div class="dropdown-divider"></div>
                                    @if( $carrier->enabled )
                                        <a class="dropdown-item text-orange" href="#" data-toggle="modal"
                                           data-target="#disableCarrierModal{{ $carrier->id }}">{{ __('Disable Carrier') }}</a>
                                    @else
                                        <a class="dropdown-item @if( $carrier->numbers->where('enabled', 1)->count() == 0 ){{ ' disabled ' }}@endif"
                                           href="#" data-toggle="modal"
                                           @if( $carrier->numbers->where('enabled', 1)->count() == 0 ){{ ' tabindex="-1" aria-disabled="true" ' }}@endif data-target="#enableCarrierModal{{ $carrier->id }}">{{ __('Enable Carrier') }}</a>
                                        <div class="dropdown-divider"></div>
                                        <a class="dropdown-item text-danger" href="#" data-toggle="modal"
                                           data-target="#deleteCarrierModal{{ $carrier->id }}">{{ __('Delete Carrier') }}</a>
                                    @endif
