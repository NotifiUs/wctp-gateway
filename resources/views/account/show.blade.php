            ...
            <div class="col-md-8">
                ...
                <div class="card">
                    <div class="card-body text-center">
                        <img alt="{{ $user->name }}" title="Gravatar for {{ $user->name }}" class="rounded-circle img-thumbnail shadow-sm" src="https://www.gravatar.com/avatar/{{ md5( $user->email )  }}?d=retro" />
                        <h3 class="display-4 text-primary mb-3">
                            <small>
                                {{ $user->name }}
                                @if( $user->email_verified_at )
                                    <i title="Verified Email" class="fas fa-check-circle text-success" style="font-size:2rem;"></i>
                                @else
                                    <a class="btn-link" href="/account/verify-email">
                                        <i title="Verify your email" class="fas fa-exclamation-circle text-danger" style="font-size:2rem;"></i>
                                    </a>
                                @endif
                            </small>
                        </h3>
